...
def run_config(config: ConverterConfig) -> bool:
...
def run_converter_job(job: ConverterJob):
    ...
    if job.is_dir:
        config_list += [
            ...
            ) for file in collect_files(job.input_path, job.is_recursive)
        ]
        success_count: int = 0
        failure_count: int = 0
        lock = Lock()
        ...
        print(f"Job {job.jobname} {"converted successfully" if run_config(config) else "failed to convert"}")
